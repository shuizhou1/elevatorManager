...
export default {
  ...
  methods: {
    ...
    getLine() {
      ...
      this.option.xAxis.data = this.date;
      this.option.series = this.data;
      this.chartLine = echarts.init(document.getElementById("J_chartLineBox"),'light');
      this.chartLine.setOption(this.option);
      window.addEventListener("resize", () => {
        this.chartLine.resize();
      });
    ...
    getChartBar() {
      ...
      barData.forEach(item => {
        this.barDataName.push(item.statusName);
        this.barValue.push(item.count);
      ...
      this.chartBarOption.xAxis[0].data = this.barDataName;
