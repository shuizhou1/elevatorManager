...
<template>
  <div v-loading="loading">
    ...
    <!-- <div class="chart_title">日困人故障率统计</div> -->
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        .then(({ data }) => {
          ...
          data.list.forEach(item => {
            keys.push(item.date);
            let rate = item.faultRatio;
            if (String(rate).length >= 6) {
              rate = rate.toFixed(4);
            }
            values.push(rate);
          });
        ...
        .then(({ data }) => {
          ...
          if (list) {
            ...
            list.forEach((item, index) => {
              let [xData, sData] = [[], []];
              item.troubleRate.forEach(value => {
                xData.push(value.date);
                let rate = value.faultRatio;
                if (String(rate).length >= 6) {
    ...
    getDataList() {
      this.$http({
        url: this.$http.adornUrl("/statistics/statistics/allTroubleRate"),
        method: "get",
        params: this.$http.adornParams({
          page: this.pageIndex,
          limit: this.pageSize,
