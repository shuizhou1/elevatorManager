<template>
  <el-dialog
    ...
    :v-loading="loading"
  >
    <div slot="title" class="dialog_headslot">{{!this.formData.id ? '维保单位' : '维保单位'}}
        ...
        if (this.formData.id) {
          ...
          this.$http({
            ...
            if (data && data.code == 0) {
              this.loading = false;
              console.log(data);
             this.formData=data.maintenanceUnit;
            }
          });
        } 
      });
    ...
    dataFormSubmit() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/maintenanceunit/maintenanceunit/${!this.formData.id ? "save" : "update"}`
            ),
            method: "post",
